...
reset_user() {
    confirm "Are you sure you want to reset the username and password to admin do you" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "x-ui stop success with xray"
        else
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        LOGI "x-ui Rebooted with xray successfully"
    else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/blackestwhite/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
ssl_cert_issue() {
    ...
    LOGI "2.know your Cloudflare Global API Key"
